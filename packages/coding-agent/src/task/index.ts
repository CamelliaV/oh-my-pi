...
import { reserveStructuredSubagentId, resolveEffectiveSubagentPolicy, runStructuredSubagent, StructuredSubagentError } from "./structured-subagent";
		...
		return manager.register(
			...
			async ({ jobId, signal: runSignal, reportProgress, markRunning }) => {
				...
				try {
					...
					progress.execution = singleResult?.execution ?? transitionSubagentExecution(
						progress.execution!, resultFailed ? "failed" : "completed", resultFailed ? finalText : undefined,
					);
					publishSubagentProgress(this.session, progress, { parentToolCallId: toolCallId, detached: true });
					...
					progress.execution = transitionSubagentExecution(progress.execution!, "failed", error instanceof Error ? error.message : String(error));
					publishSubagentProgress(this.session, progress, { parentToolCallId: toolCallId, detached: true });
					onSettled?.(true);
	...
	async #runSyncSpawns(args: {
		...
		const { results } = await mapWithConcurrencyLimitAllSettled(
			...
			async (spawn, _position, workerSignal) => {
				...
				const id = spawn.preAllocatedId ?? await reserveStructuredSubagentId(this.session, { label: spawn.item.name });
				...
				try {
					...
					queued.execution = transitionSubagentExecution(queued.execution!, workerSignal.aborted ? "cancelled" : "failed", String(error));
					publishSubagentProgress(this.session, queued, { parentToolCallId: toolCallId, detached: false });
					onItemProgress?.(spawn.index, queued);
		...
		try {
			...
			if (latestProgress?.execution) {
				latestProgress = {
					...
					execution: transitionSubagentExecution(latestProgress.execution, signal?.aborted ? "cancelled" : "failed", message),
				};
