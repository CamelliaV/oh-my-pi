...
function formatFreshSessionResult(result: FreshSessionResult): string {
	const stateLabel =
		result.closedProviderSessions === 1 ? "provider state" : "provider states";
...
async function generateRenameTitle(
...
function parseShakeMode(args: string): ShakeMode | { error: string } {
	...
	return {
		error: `Unknown /shake mode "${verb}". Use elide, images, or thinking.`,
	};
}

/** Format the session's workspace directories (cwd + additional) for display. */
function formatWorkspaceDirectories(
...
async function fatalMoveFailure(
	text: string,
	runtime: SlashCommandRuntime,
...
async function relocateHeadlessSession(
	...
	try {
		...
		return usage(
			`Failed to save pending settings: ${errorMessage(err)}`,
			runtime,
		);
	}
...
export const BUILTIN_LIFECYCLE_SLASH_COMMANDS: ReadonlyArray<SlashCommandSpec> =
	[
		...
		},
		{
			...
			getTuiAutocompleteDescription: (runtime) =>
				runtime.ctx.session.isStreaming
					? "Fresh: unavailable while streaming"
					: "Fresh: ready",
		...
		},
		{
			...
			getTuiAutocompleteDescription: (runtime) =>
				runtime.ctx.session.isStreaming
					? "Clear: unavailable while streaming"
					: "Clear: drop context, keep session",
			handleTui: async (_command, runtime) => {
				runtime.ctx.editor.setText("");
		...
		},
		{
			...
			getTuiAutocompleteDescription: (runtime) => {
				...
				return usage
					? `Compact: context ${Math.round(usage.percent)}% used`
				...
				const runCompact = async (): Promise<void> => {
					...
					try {
						await runtime.session.compact(
							parsed.instructions,
							parsed.mode ? { mode: parsed.mode } : undefined,
						);
					} catch (err) {
						...
						if (
							...
							err.cause === USER_INTERRUPT_LABEL
						)
							return;
					...
					if (beforeTokens != null && afterTokens != null) {
						...
						await runtime.output(
							`Compaction complete. Tokens: ${beforeTokens} -> ${afterTokens} (saved ${saved}).`,
						);
					} else {
			...
			handleTui: async (command, runtime) => {
				...
				await runtime.ctx.handleCompactCommand(
					parsed.instructions,
					parsed.mode,
				);
			},
		},
		{
			...
			subcommands: [
				{
					name: "elide",
					description: "Strip tool results + large blocks (default)",
				},
		...
		},
		{
			...
			subcommands: [
				...
				{
					name: "queue",
					description: "Show pending memory deltas awaiting consolidation",
				},
				...
				{
					name: "mm history",
					description: "Diff the change history of a mental model",
				},
				{
					name: "mm seed",
					description: "Create any built-in mental models that are missing",
				},
				{
					name: "mm delete",
					description: "Delete a mental model from the bank (id required)",
				},
				{
					name: "mm reload",
					description: "Re-pull the cached <mental_models> block",
				},
			],
			allowArgs: true,
			handle: async (command, runtime) => {
				...
				switch (verb) {
					...
					case "queue": {
						...
						await runtime.output(
							payload ??
								`Memory queue is not available for the ${backend.id} backend.`,
						);
						return commandConsumed();
					}
					case "sync": {
					...
					case "diagnose": {
						...
						await runtime.output(
							payload ?? memoryStatsUnavailableMessage(backend.id, verb),
						);
						return commandConsumed();
					}
					case "mm":
		...
		},
		{
			...
			handle: async (command, runtime) => {
				...
				const runRename = async (): Promise<void> => {
					...
					try {
						const generation =
							command.args || generateRenameTitle(session, runtime.signal);
						titleRevision = sessionManager.titleRevision;
						const title =
							typeof generation === "string" ? generation : await generation;
		...
		},
		{
			...
			handle: async (command, runtime) => {
				if (runtime.session.isStreaming)
					return usage("Cannot move while streaming.", runtime);
		...
		},
		{
			...
			handle: async (command, runtime) => {
				if (runtime.session.isStreaming)
					return usage("Cannot create a worktree while streaming.", runtime);
				...
				try {
					...
					return usage(
						`Worktree creation failed: ${errorMessage(err)}`,
						runtime,
					);
				}
				const failure = await relocateHeadlessSession(runtime, worktree.path);
				if (failure) return failure;
				const cleanup = await cleanSourceCheckoutIfConfigured(
					sourceCwd,
					runtime.settings,
				);
				...
				await runtime.output(
					formatSessionWorktreeSummary(worktree, cleanup.cleaned),
				);
				return commandConsumed();
			},
			handleTui: async (command, runtime) => {
		...
		},
		{
			...
			handle: async (command, runtime) => {
				if (runtime.session.isStreaming)
					return usage("Cannot add a directory while streaming.", runtime);
				if (!command.args)
					return usage(
						formatWorkspaceDirectories(runtime, "Usage: /add-dir <path>"),
						runtime,
					);
				...
				try {
					...
					if (!stat.isDirectory())
						return usage(`Not a directory: ${resolved}`, runtime);
				} catch {
				...
				await runtime.session.refreshBaseSystemPrompt();
				await runtime.output(
					formatWorkspaceDirectories(runtime, `Added ${added}.`),
				);
				return commandConsumed();
			},
		},
		{
			...
			handle: async (command, runtime) => {
				if (runtime.session.isStreaming)
					return usage("Cannot remove a directory while streaming.", runtime);
				...
				try {
					removed =
						await runtime.sessionManager.removeWorkspaceDirectory(resolved);
				} catch (err) {
					return usage(errorMessage(err), runtime);
				}
				if (removed === null) {
				...
				await runtime.session.refreshBaseSystemPrompt();
				await runtime.output(
					formatWorkspaceDirectories(runtime, `Removed ${removed}.`),
				);
				return commandConsumed();
			},
		},
		...
		},
		{
			name: "wiki",
			icon: "memory",
			description: "Compile the external wiki vault",
			subcommands: [
				{ name: "compile", description: "Run one bounded smol compile" },
			],
			allowArgs: true,
			handle: async (command, runtime) => {
				if (command.args.trim() !== "compile")
					return usage("Usage: /wiki compile", runtime);
				await runtime.output(await runExternalWikiCompile(runtime.session));
				return commandConsumed({ agentInvoked: true });
			},
			handleTui: async (command, runtime) => {
				runtime.ctx.editor.setText("");
				if (command.args.trim() !== "compile") {
					runtime.ctx.showError("Usage: /wiki compile");
					return;
				}
				runtime.ctx.showStatus(
					"Wiki compile started. Open Agent Hub to watch it.",
				);
				try {
					runtime.ctx.showStatus(
						await runExternalWikiCompile(runtime.ctx.session),
					);
				} catch (error) {
...
async function rescopeHeadlessToCwd(
	runtime: SlashCommandRuntime,
	cwd: string,
): Promise<void> {
	setProjectDir(cwd);
