...
export class CredentialPool implements CredentialsApi {
	...
	/**
	 * Reconcile a models.yml `apiKeys` pool. Rows match by exact key, so an
	 * unchanged re-sync keeps row ids and the rate-limit blocks keyed to them.
	 * Returns whether membership changed.
	 */
	async syncConfigPool(provider: string, keys: readonly string[]): Promise<boolean> {
		const wanted = new Set(keys);
		const rows = this.reloadProvider(provider).filter(
			(row): row is StoredAuthCredential & { credential: ApiKeyCredential } =>
				row.credential.type === "api_key" && row.credential.source === "config",
		);
		const present = new Set(rows.map(row => row.credential.key));
		let changed = false;
		for (const key of keys) {
			if (present.has(key)) continue;
			await this.#store.upsertAuthCredential(provider, { type: "api_key", key, source: "config" });
			changed = true;
		}
		for (const row of rows) {
			if (wanted.has(row.credential.key)) continue;
			await this.#store.deleteAuthCredential(row.id, "removed from models.yml apiKeys");
			changed = true;
		}
		if (changed) this.reloadProvider(provider);
		return changed;
	}

	/** Providers whose stored rows include a config-sourced api key. */
	configPoolProviders(): string[] {
		const providers: string[] = [];
		for (const row of this.#store.listAuthCredentials()) {
			if (row.credential.type !== "api_key" || row.credential.source !== "config") continue;
			if (!providers.includes(row.provider)) providers.push(row.provider);
		}
		return providers;
	}

	/** Soft-delete every config-sourced pool row of `provider`. */
	async removeConfigPool(provider: string): Promise<boolean> {
		const rows = this.reloadProvider(provider).filter(
			(row): row is StoredAuthCredential & { credential: ApiKeyCredential } =>
				row.credential.type === "api_key" && row.credential.source === "config",
		);
