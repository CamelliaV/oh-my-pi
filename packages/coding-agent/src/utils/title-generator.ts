...
export type TerminalTitleSpinnerStyle = "pie" | "braille" | "pulse" | "dots" | "line";

/**
 * Working-state spinner frames per `tui.titleSpinner` style. `pie` (the fork
 * default) is a nerd-font pie-ring sweep — single column, smooth rotation,
 * needs PUA glyph coverage; `braille` is the historical upstream default;
 * `pulse` fills and empties a moon; `dots` cycles single braille dots; `line`
 * is plain ASCII (`- \ | /`) for fonts without braille coverage. Every frame
...
/** The pie ring reads best one notch slower than the dense braille sweep. */
const TITLE_SPINNER_INTERVAL_MS: Record<TerminalTitleSpinnerStyle, number> = {
	pie: 130,
	braille: 80,
	pulse: 80,
	dots: 80,
...
function startTerminalTitleSpinner(): void {
	...
	terminalTitleRuntime.timer = setInterval(() => {
		...
		emitTerminalTitle();
	}, TITLE_SPINNER_INTERVAL_MS[terminalTitleRuntime.style]);
