...
export class SessionMaintenance {
	...
	/** Context occupancy shown to callers. */
	getContextUsage(options?: { contextWindow?: number }): ContextUsage | undefined {
	...
	/** Whether the active automatic compaction route prefers the provider-native remote method. */
	usesCodexRemoteAutoCompaction(): boolean {
		return resolveSpeculationMethod(this.#model, this.#host.settings.getGroup("compaction")) === "remote";
	}
	...
	async #commitCompactionEntry(args: {
		...
		const entryId = this.#host.sessionManager.appendCompaction(
			...
			args.tokensBefore,
			{
				...
				requestUsage: args.requestUsage,
				method: args.method,
		...
		try {
			...
			if (armedSpec) {
				...
				return await this.#commitAutoCompactionResult({
					...
					requestUsage: armedSpec.result.requestUsage,
					fromExtension: false,
