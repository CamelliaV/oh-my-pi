...
export class EventController {
	...
	async #handleMessageStart(event: Extract<AgentSessionEvent, { type: "message_start" }>): Promise<void> {
		...
		if (event.message.role === "hookMessage" || event.message.role === "custom") {
			...
			vocalizer.clear();
			// Only genuinely user-attributed prompts anchor the delta and open a
			// work window; a mid-run agent-attributed `user` message (advisor
			...
			if (userText && event.message.synthetic !== true) {
				...
				if (renderToolName === "read") {
					...
					if (readArgsCollapseIntoGroup(content.arguments)) {
						...
						if (existing) {
							...
							existing.updateIntent(intent, content.id);
