...
import { classifyGroupedLines } from "./grouped-file-output";
import {
	formatExpandHint,
	formatMoreItems,
	formatStatusIcon,
	...
	return framedToolCard(theme, () => {
		...
		return {
			...
			sections: [
				...
				{ label: theme.fg("toolTitle", label), content: bodyLines },
			],
			applyBg: false,
		};
	});
...
function parseDiagnosticItems(lines: string[]): DiagnosticItem[] {
	...
	for (let index = 0; index < lines.length; index++) {
		...
		if (context?.kind === "dir") continue;
		if (context?.kind === "file") {
			currentFile = context.headerPath ?? context.filePath;
			continue;
		}

		const item =
			parseDiagnosticMessage(line.trim()) ??
...
function parseGroupedDiagnosticLine(line: string, file: string | undefined): ParsedDiagnostic | null {
	if (!file) return null;
	return parseDiagnosticMessage(`${file}:${line.trim()}`);
}
