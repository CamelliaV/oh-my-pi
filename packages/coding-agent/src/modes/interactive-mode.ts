...
/**
 * HUD visibility = upstream's active-only rows plus the fork's retention rule:
 * a detached subagent that failed / was aborted / reached a terminal execution
 * phase or is a historical recovery row stays pinned until Main disposes it,
 * instead of vanishing from the HUD the moment it leaves "active".
 */
function isHudPinnedSubagent(session: ObservableSession): boolean {
	if (isHudSubagent(session)) return true;
	if (session.kind !== "subagent" || session.detached !== true) return false;
	return (
		session.status === "failed" ||
		session.status === "aborted" ||
		session.execution?.phase === "failed" ||
		session.execution?.phase === "cancelled" ||
...
export function renderSubagentHudLines(sessions: ObservableSession[], columns: number, expanded = false): string[] {
	const running = sessions.filter(isHudPinnedSubagent);
	...
	const rows = renderTreeList(
		{
			...
			renderItem: (session, context) => {
				...
				const view = presentSubagentExecution(
					{
						id: session.id,
						execution: session.execution ?? session.progress?.execution,
				...
				let line = `${dot} ${modelLead}${theme.fg(color, theme.bold(displayId))}${badge}`;
				const description = session.description?.trim() || session.progress?.description?.trim();
				// Fork: with no distinct spawn description, surface the task or the
				// execution summary so failed/retained rows keep their state visible.
				const text =
				...
				if (budget > 0 && text) {
					const formatted = replaceTabs(text).replace(/\s*[\r\n]+\s*/g, " ↵ ");
					line += `${theme.fg(color, ":")} ${theme.fg(color, truncateToWidth(formatted, budget))}`;
				}
	...
	const unresolved = running.filter(
		session =>
			session.status === "failed" ||
			session.status === "aborted" ||
			session.execution?.phase === "failed" ||
	...
	return [
		"",
		truncateToWidth(theme.bold(theme.fg(unresolved > 0 ? "warning" : "accent", header)), columns),
...
export class InteractiveMode implements InteractiveModeContext {
	 ...
	 * on spawn. Retained failed/aborted/historical rows stay pinned until Main
	 * disposes them; the block clears once nothing matches the HUD predicate.
	 * The click row map MUST filter with the same predicate as the line builder.
	 */
