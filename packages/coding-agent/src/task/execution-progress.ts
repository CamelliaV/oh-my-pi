...
export function subagentFailureFromJob(
	job: AsyncJob | undefined,
): { id: string; execution: SubagentExecutionState } | undefined {
...
export function markSubagentFailureDelivered(
	...
	if (
		!current ||
		current.run !== failure.execution.run ||
		current.stoppedAt !== failure.execution.stoppedAt ||
		current.phase !== "failed" ||
		current.notification?.state === "delivered"
	)
		return;
	const now = Math.max(Date.now(), current.updatedAt + 1);
	recordSubagentExecution(registry, failure.id, {
		...current,
		updatedAt: now,
		notification: { state: "delivered", at: now },
	});
}
