...
export function presentSubagentExecution(
	...
	const snapshotNow = execution && (!active || input.historical) ? Math.min(now, execution.stoppedAt ?? execution.updatedAt) : now;
	...
	const color: ExecutionViewColor = failed ? "error" : completed ? "success" : abnormal ? "warning" : active ? "accent" : "dim";
	...
	const add = (text: string): void => { details.push(executionDisplayText(text, 180)); };
	...
	if (retryAt !== undefined) add(`下次重试：${clock(retryAt)}${retryAt > now ? `（${duration(retryAt - now)} 后）` : "（等待实际启动）"}`);
	if (execution && (expanded || abnormal) && execution.retries > 0) {
		const waiting = retry ? Math.min(Math.max(0, now - retry.startedAt), Math.max(0, retry.retryAt - retry.startedAt)) : 0;
	...
	if (execution && (expanded || abnormal)) {
		...
		if (expanded || silent) add(`无新活动 ${duration(activityAge)}${silent && activityAge >= STALE_HINT_MS ? " · 疑似停滞，尚未确认" : ""}`);
	}
	if (failed || stoppedHistory) {
		const stoppedAge = execution?.stoppedAt !== undefined ? duration(now - execution.stoppedAt) : "unknown duration";
		add(stoppedHistory ? `历史快照不代表仍在运行；已停止 ${stoppedAge}，等待 Main 处理` : `已停止 ${stoppedAge}，等待 Main 处理`);
		add(`可对 Main 说「继续 ${executionDisplayText(input.id, 64)}」，由 Main 沿用原会话继续`);
		if (execution?.notification) add(execution.notification.state === "delivered" ? "失败通知已送达 Main；等待其决定" : "失败通知待送达 Main");
	}
	...
	if (expanded && execution) {
		...
		if (execution.notification) add(`${execution.notification.state === "delivered" ? "已通知 Main" : "等待通知 Main"} · ${clock(execution.notification.at)}`);
		if (execution.activity && abnormal) add(`活动：${execution.activity}`);
	} else if (expanded && !execution) {
		add("旧记录未保存阶段与重试历史；仅显示已有状态");
	}
	const events = expanded && execution
		? execution.events.slice(-MAX_EVENT_ROWS).map(event => executionDisplayText(`${clock(event.at)} · ${PHASE_LABELS[event.phase]}${event.detail ? ` · ${event.detail}` : ""}`, 180))
		: [];
	return { phase, label, color, active, failed, abnormal, summary: executionDisplayText(summaryParts.join(" · ")), details, events };
}
