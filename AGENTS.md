...
30. `feat(task)` subagent execution-state visibility + `hub resume` explicit continuation (immutable phase machine, queued/failed rows retained in the HUD, recovery lineage across follow-up turns) — `task/{execution-state,execution-progress,execution-view,executor,index,render,structured-subagent,types}.ts, tools/hub/{resume,index,messaging,types}.ts, registry/{agent-registry,persisted-agents}.ts, modes/{interactive-mode,session-observer-registry,components/agent-hub}.ts, prompts/tools/{hub,task}.md`
31. `fix(session)` mid-stream transport death after committed text resumes instead of pinning the error (bounded continuation counter, hidden developer resume directive, socket-close wording added to mid-stream-death family) — `session/turn-recovery.ts, session/agent-session.ts, prompts/system/partial-stream-resume.md, test/partial-stream-death.test.ts`
32. `feat(tui)` markdown code blocks render as rounded boxes with a line-number gutter (language on the top bar, `boxRound` glyphs, `mdCodeBlockBorder` frame; `codeBlockIndent` still governs list-item code; bare-fence fallback under ~15 columns) — `packages/tui/src/components/markdown.ts, docs/theme.md, packages/tui/test/{markdown,markdown-tree-wrap}.test.ts`