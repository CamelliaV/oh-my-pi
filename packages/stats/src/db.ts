...
export function insertMessageStats(stats: MessageStats[]): number {
	...
	const stmt = db.prepare(`
		...
		WHERE messages.premium_requests < excluded.premium_requests
			...
			OR (excluded.cost_input IS NOT messages.cost_input)
			OR (excluded.cost_output IS NOT messages.cost_output)
			OR (excluded.cost_cache_read IS NOT messages.cost_cache_read)
			OR (excluded.cost_cache_write IS NOT messages.cost_cache_write)
