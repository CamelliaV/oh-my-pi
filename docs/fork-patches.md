...
Two-stage synthetic-commit merge: `upstream/v18.1.6` (2a031c84, 743 commits,
1869 files) resolved first on `trial/v18.1.6` (40 conflicts / 23 files), then
`upstream/v18.1.10` (c3eb181, 114 commits, 541 files, parented on 2a031c84)
merged incrementally — only 8 new conflicts across 5 files. All 21 patches
survive; the adjudications that matter:

1. **compat rewrite** — upstream moved `buildAnthropicCompat` →
   `compat/resolve.ts` `resolveAnthropicPolicy` (identity-facts/rules-driven,
   dropped model-name matching incl. the Z.AI `requiresToolResultId`
   special case). Patch 19 ported as `extraBetas: []` in the object literal
   only; `ResolvedAnthropicCompat = Required<Omit<…>>` inherits the field.
2. **openai-completions ladder** — fork's always-thinking strip stays FIRST;
   upstream's effort-fallback dedup ladder follows; ours'
   `isOpenRouterAnthropicModel` special case replaced by the generic
   `model.compat.retryWithoutStrictOnGrammarError` (rule-owned via catalog
   wire axes).
3. **patch 21 re-port** — upstream's rewritten caching kept the string branch,
   so the re-land was a normal 3-way conflict: `applyPromptCaching` adopts
   upstream's `eligibleMessages`/`clear_at` loop (deferred tools) with
   block-only content handling; `convertAnthropicMessages` adopts
   `hasProviderControls` empty-content semantics while wrapping non-empty
   strings into `[{type:"text"}]`.
4. **reaction badge** — upstream 18.1.6 rewrote the user bubble borderless
   with a top-padding badge row; the frame integration splices a badge row
   below the top border in `render()` instead. The first attempt used
   `#frame.children.unshift` + `#frame.invalidate?.()` — Box implements NO
   public `invalidate` (only private `#invalidateCache`), so `Component`'s
   optional hook made it a compile-legal no-op and the badge never rendered;
   the render()-splice needs no child surgery at all. Driver-verified.
5. **image budget** — fork per-screen ledgers (`#transmittedMain/Alt`)
   unioned with upstream's keyed `Map` transmit queue + cancel-on-purge;
   `tui.ts` alt-exit takes upstream's deferred-exit branch plus the
   unconditional `setScreen("main")` (the fork comment had already blessed
   the deferred variant). ImageBudget driver 5/5.
6. **compaction** — upstream 18.1.10's Codex V2 request refactor
   (`buildCompactionV2RequestFromBody`, prompt-cache reuse fix #10786) kept
   the fork's `requestTimestamp`/`requestUsage` tracking verbatim.
7. **mcp/manager** — fork lazy-gateway fields (`#lazyHeld`/`#lazySources`)
   unioned with upstream's browser-filter machinery; `LoadMCPConfigsResult`
   keeps the fork's `lazyConfigs`/`lazySources` required fields, so upstream's
   new `sdk-computer-prelude-toggle` test needed its fixture loader extended.
8. **search affinity** — `isClaudeModelId` inlined as the namespace regex
   (upstream deleted identity/family.ts); `parseOpenAIModel` replaced by
...
upstream-new tests (18.1.6→18.1.10 diff) incl. `read-image-question`,
