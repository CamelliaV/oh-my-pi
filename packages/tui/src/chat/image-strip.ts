...
export interface ImageStripOptions {
	...
	requestRender?: () => void;
	/** When false, the strip renders nothing. Hosts wire this to terminal.showImages. */
	showImages?: boolean;
	/** Kitty-only PNG conversion. Absent, non-PNG payloads stay a text placeholder. */
	convertToPng?: (image: ImageContent) => Promise<ImageContent>;
...
export class ImageStrip implements Component {
	...
	get conversionsPending(): number {
		...
		if (!this.#options.budget || this.#options.showImages === false) return 0;
	...
	render(width: number): readonly string[] {
		...
		if (this.#options.showImages === false) return [];
	...
	#convertKittyImages(images: readonly ImageContent[]): void {
		...
		for (const image of images) {
			...
			const convert = this.#options.convertToPng;
			if (!convert) return;
