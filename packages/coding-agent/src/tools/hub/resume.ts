...
import { createQueuedSubagentProgress, publishSubagentProgress, recordSubagentExecution } from "../../task/execution-progress";
...
export async function executeResume(
	...
	if (!registry || !to || to === "all" || to === senderId || !message) {
		return hubErrorResult("resume requires one subagent id in `to` and continuation instructions in `message`.", { op: "resume", from: senderId, to });
	}
	if (session.getPlanModeState?.()?.enabled || session.restrictToolNames) {
		return hubErrorResult("Subagent continuation is unavailable in plan mode or a restricted session.", { op: "resume", from: senderId, to });
	}
	...
	if (!ref || ref.kind !== "sub" || !isCurrentSessionRosterRef(ref, root)) {
		return hubErrorResult(`No resumable subagent "${to}" belongs to this session. Use hub list to identify it.`, { op: "resume", from: senderId, to });
	}
	if (ref.status === "aborted") {
		return hubErrorResult(`子任务 ${to} 已被明确终止，原会话不可续跑。可读取 history://${to} 后重新派发；不得把替代任务称为原会话恢复。`, { op: "resume", from: senderId, to });
	}
	const previous = ref.history?.execution ?? createSubagentExecution(ref.createdAt);
	if (previous.recovery && !previous.recovery.startedAt && (previous.phase === "queued" || previous.phase === "creating")) {
		return { content: [{ type: "text", text: `${to} 的续跑已在安排中，未重复提交。当前阶段：${previous.phase}。` }], details: { op: "resume", from: senderId, to, execution: previous } };
	}
	if (ref.status === "running" || ref.session?.isStreaming) {
		return { content: [{ type: "text", text: `${to} 已在运行，未创建第二次运行。如需补充指令，请使用 hub send。` }], details: { op: "resume", from: senderId, to, execution: ref.history?.execution } };
	}
	...
	try {
		...
		if (!current || (current.run === requested.run && current.recovery?.requestedAt === requested.recovery?.requestedAt)) {
		...
		return hubErrorResult(`无法继续 ${to}：${reason}\n原任务未恢复。读取 history://${to} 检查原因；确需替代任务时明确重新派发。`, { op: "resume", from: senderId, to, execution: registry.get(to)?.history?.execution });
	}
}
