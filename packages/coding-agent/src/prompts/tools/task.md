...
`completed`: yielded successfully; claimed artifacts unverified.
A failed unfinished child needs Main's disposition. Continue it with `write` to `agent://<id>?resume=1` and continuation instructions as `content`; do not spawn a replacement or claim it is running before the runtime confirms.
{{/if}}
