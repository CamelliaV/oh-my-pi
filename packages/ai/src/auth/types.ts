...
/** Selection policy for a models.yml `apiKeys` pool. */
...
export interface KeysApi {
	 ...
	 * `providers.<name>.apiKeys`). Keys match existing rows by exact value so
	 * re-sync preserves row ids and the persisted blocks keyed to them. Policy
	 * defaults to `first-fill`.
	 */
	setConfigPool(provider: string, keys: readonly string[], policy?: ApiKeyPoolPolicy): Promise<void>;
	/**
	 * Tear down config-sourced pools for providers absent from `activeProviders`.
	 * Called after a config (re)load with the providers whose `apiKeys` were
	 * just synced. Rows come from the persistent store, so pools configured
	 * before a process restart are still pruned.
	 */
	pruneConfigPools(activeProviders: ReadonlySet<string>): Promise<void>;
	/**
