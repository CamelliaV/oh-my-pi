	...
	*#renderSessionContextSteps(
		...
		for (let i = 0; i < count; i++) {
			...
			if (message.role === "assistant") {
				...
				if (assistantUsageIsBilled(message.usage)) workUsage.add(message);
				...
				// Only genuinely user-attributed prompts anchor the delta and open
				// a work window; a mid-run agent-attributed `user` message (advisor
				// tool-loop redirect) must not reset either.
				...
				this.ctx.addMessageToChat(
					message,
					userSessionUsage
						? { reuseSettledComponent: options.reuseSettledComponents, sessionUsage: userSessionUsage }
