...
export interface RateLimitsDeps {
	...
	/** Session-hash start index, shared with credential selection. */
...
export class RateLimits implements LimitsApi {
	...
	async #resolveCredentialTarget(
		...
		// keys.get would start on, even if a later resolve already skipped it.
		if (!sessionId) return undefined;
		const apiKeys = this.#deps.pool
			.credentials(provider)
			.map((credential, index) => ({ credential, index }))
			.filter(
				(entry): entry is { credential: ApiKeyCredential; index: number } => entry.credential.type === "api_key",
			);
		const loginKeys = apiKeys.filter(entry => entry.credential.source === "login");
		const pool = loginKeys.length > 0 ? loginKeys : apiKeys;
		...
		// the pool is already blocked. Otherwise keys.get may have skipped the
		// home key and the request used a sibling — burning home again would
		// leave the actual exhausted bearer in rotation.
		if (pool.some(entry => this.#deps.blocks.isBlocked(provider, providerKey, entry.index))) {
			return undefined;
		}
		const start = this.#deps.hashedIndex(sessionId, pool.length);
