			...
			this.#eventBusUnsubscribers.push(
				bus.on(
					...
					dedupe(data => {
						...
						if (existing) {
							const execution = latestSubagentExecution(payload.execution, existing.execution, existing.progress?.execution);
							if (payload.execution && execution !== payload.execution) return;
							const nextStatus = execution ? executionStatus(execution) : status;
							if (nextStatus === "active" && (existing.status !== "active" || (execution && execution.run > (existing.progress?.execution?.run ?? -1)))) existing.progress = undefined;
			...
			this.#eventBusUnsubscribers.push(
				bus.on(
					...
					dedupe(data => {
						...
						this.#ensureParentSortOrder(payload.parentToolCallId, sortOrder);
						if (existing) {
							const execution = latestSubagentExecution(progress.execution, existing.execution, existing.progress?.execution);
