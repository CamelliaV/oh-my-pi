...
export async function runStructuredSubagent(request: StructuredSubagentRequest): Promise<StructuredSubagentResult> {
	...
	try {
		...
		pending.execution = transitionSubagentExecution(
			request.execution ?? pending.execution!,
			"creating",
			"初始化会话与工具",
		);
		baseOptions.execution = pending.execution;
		request.onProgress?.(pending);
		publishSubagentProgress(request.session, pending, {
			parentToolCallId: request.parentToolCallId,
			detached: request.detached,
		});
		...
		if (result.execution) {
			result.execution = transitionSubagentExecution(
				result.execution,
				completedSuccessfully ? "completed" : result.aborted ? "cancelled" : "failed",
				completedSuccessfully ? "结果已完成" : result.error,
			);
		}
		return {
			result,
