...
export class KeyOverrides {
	...
	/** True when a models.yml `apiKeys` pool is installed for this provider. */
	hasConfigPool(provider: string): boolean {
		return this.#configPoolPolicies.has(provider);
	}

	configPoolPolicy(provider: string): ApiKeyPoolPolicy | undefined {
		return this.#configPoolPolicies.get(provider);
	}

	setConfigPool(provider: string, policy: ApiKeyPoolPolicy): void {
		this.#configPoolPolicies.set(provider, policy);
	}

	forgetConfigPool(provider: string): void {
		this.#configPoolPolicies.delete(provider);
	}

	configPoolProviders(): IterableIterator<string> {
		return this.#configPoolPolicies.keys();
	}
...
export class KeyCascade implements KeysApi {
	...
	source(provider: string, { env = "dedicated" }: AuthSourceOptions = {}): AuthSource | undefined {
		...
		if (this.#deps.overrides.hasConfigPool(provider)) return { kind: "config", concrete: true };
	...
	async peek(provider: string): Promise<string | undefined> {
		...
		if (this.#deps.overrides.hasConfigPool(provider)) {
			const pooled = this.#deps.selector.selectByType(
				provider,
				"api_key",
				undefined,
				credential => credential.type === "api_key" && credential.source === "config",
				this.#deps.overrides.configPoolPolicy(provider) === "round-robin" ? undefined : 0,
			);
			if (pooled) return this.#deps.overrides.resolve(pooled.credential.key);
		}
	...
	async get(
		...
		if (this.#deps.overrides.hasConfigPool(provider)) {
			const pooled = await this.#deps.selector.selectApiKey(
				provider,
				this.#deps.overrides.configPoolPolicy(provider) === "round-robin" ? undefined : sessionId,
				options,
				credential => credential.source === "config",
				this.#deps.overrides.configPoolPolicy(provider) === "round-robin" ? undefined : 0,
			);
			if (pooled) {
				this.#deps.affinity.record(provider, sessionId, "api_key", pooled.index);
				const credentialId = onCredentialId ? this.#deps.pool.entries(provider)[pooled.index]?.id : undefined;
				const apiKey = await this.#deps.overrides.resolve(pooled.credential.key);
	...
	describe(provider: string, sessionId?: string): string | undefined {
		...
		if (this.#deps.overrides.hasConfigPool(provider)) {
			const policy = this.#deps.overrides.configPoolPolicy(provider);
	...
	removeConfig(provider: string): void {
		...
		void this.#removeConfigPool(provider);
	}

	async setConfigPool(
		provider: string,
		keys: readonly string[],
		policy: ApiKeyPoolPolicy = "first-fill",
	): Promise<void> {
		const deduped = [...new Set(keys.filter(key => key.length > 0))];
		if (deduped.length === 0) {
			await this.#removeConfigPool(provider);
			return;
		}
		this.#deps.overrides.setConfigPool(provider, policy);
		if (await this.#deps.pool.syncConfigPool(provider, deduped)) this.#deps.pool.reset(provider);
	}

	async pruneConfigPools(activeProviders: ReadonlySet<string>): Promise<void> {
		for (const provider of this.#deps.overrides.configPoolProviders()) {
			if (!activeProviders.has(provider)) this.#deps.overrides.forgetConfigPool(provider);
		}
		let stored: string[];
		try {
			stored = this.#deps.pool.configPoolProviders();
		} catch (err) {
			logger.debug("Failed to enumerate stored credentials for config pool pruning", { err });
			return;
		}
		for (const provider of stored) {
			if (!activeProviders.has(provider)) await this.#removeConfigPool(provider);
		}
	}

	async #removeConfigPool(provider: string): Promise<void> {
		this.#deps.overrides.forgetConfigPool(provider);
		if (await this.#deps.pool.removeConfigPool(provider)) this.#deps.pool.reset(provider);
	}
