...
async function printStats(): Promise<void> {
	...
	console.log(`  Total Cost: ${formatCost(overall.totalCost, overall.unpricedRequests)}`);
	console.log(
	...
	if (byModel.length > 0) {
		...
		for (const m of byModel.slice(0, 10)) {
			...
			console.log(
				`  ${m.model}: ${formatNumber(m.totalRequests)} reqs, ${formatCost(m.totalCost, m.unpricedRequests)}, ${modelCacheRate} cache rate (${formatNumber(m.cacheTelemetryRequests)}/${formatNumber(m.cacheEligibleRequests)} reported), ${formatPercent(m.cacheSavings)} cache savings`,
			);
		}
	}
