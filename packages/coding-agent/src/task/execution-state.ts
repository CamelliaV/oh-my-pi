...
const phases = new Set<SubagentExecutionPhase>([
	"queued", "creating", "waiting-model", "responding", "tool", "waiting", "retrying", "finishing", "completed", "failed", "cancelled",
]);
...
export function createSubagentExecution(now = Date.now()): SubagentExecutionState {
	return snapshot({
		phase: "queued", since: now, updatedAt: now, run: 0, startedAt: now, lastActivityAt: now,
		retries: 0, retryWaitMs: 0, events: [{ at: now, phase: "queued" }],
	});
}
...
export function transitionSubagentExecution(
	...
	return snapshot({
		...
		...(phase === "failed" ? { lastError: activity ?? previous.lastError, notification: { state: "pending" as const, at } } : {}),
		...(phase === "cancelled" ? { lastError: activity ?? previous.lastError } : {}),
		events: [...previous.events.slice(-(MAX_EXECUTION_EVENTS - 1)), { at, phase, ...(activity ? { detail: activity } : {}) }],
	});
}
...
export function requestSubagentRecovery(
	...
	if (previous.recovery && previous.recovery.startedAt === undefined && (previous.phase === "queued" || previous.phase === "creating")) {
		return previous;
	}
	const at = Math.max(now, previous.updatedAt + 1);
	return snapshot({
...
export function startSubagentExecution(previous: SubagentExecutionState, now = Date.now()): SubagentExecutionState {
	...
	return snapshot({
		...transitionSubagentExecution(previous, "creating", recovering ? "Starting requested continuation" : "Preparing session", at),
		run: previous.run + 1,
		runStartedAt: at,
		stoppedAt: undefined,
		retry: undefined,
...
export function progressSubagentExecution(previous: SubagentExecutionState, detail: string, now = Date.now()): SubagentExecutionState {
	const at = Math.max(now, previous.updatedAt);
	return snapshot({ ...previous, lastProgressAt: at, lastProgress: detail, lastActivityAt: at, updatedAt: at, lastError: undefined, notification: undefined });
}
...
export function settleSubagentRetry(previous: SubagentExecutionState, now = Date.now()): SubagentExecutionState {
	...
	return snapshot({ ...previous, retry: undefined, retryWaitMs: previous.retryWaitMs + retryWaitMs, updatedAt: Math.max(now, previous.updatedAt) });
}
...
export function retrySubagentExecution(
	...
	if (previous.retry && previous.retry.attempt === retry.attempt && previous.retry.maxAttempts === retry.maxAttempts && previous.retry.error === retry.error) return previous;
	...
	return snapshot({
		...
		retry: { attempt: retry.attempt, maxAttempts: retry.maxAttempts, startedAt: at, retryAt: at + Math.max(0, retry.delayMs), error: retry.error },
	});
}
...
export function readSubagentExecution(value: unknown): SubagentExecutionState | undefined {
	...
	if (!data || !phase(data.phase) || !time(data.since) || !time(data.updatedAt) || !count(data.run) || !time(data.startedAt) || !time(data.lastActivityAt) || !count(data.retries) || !time(data.retryWaitMs) || !Array.isArray(data.events)) return undefined;
	...
	for (const value of data.events) {
		...
		if (!event || !time(event.at) || !phase(event.phase) || (event.detail !== undefined && typeof event.detail !== "string")) return undefined;
		events.push({ at: event.at, phase: event.phase, ...(event.detail !== undefined ? { detail: event.detail } : {}) });
	}
	let retry: SubagentExecutionState["retry"];
	if (data.retry !== undefined) {
		const item = record(data.retry);
		if (!item || !count(item.attempt) || !count(item.maxAttempts) || !time(item.startedAt) || !time(item.retryAt) || item.retryAt < item.startedAt || typeof item.error !== "string") return undefined;
		retry = { attempt: item.attempt, maxAttempts: item.maxAttempts, startedAt: item.startedAt, retryAt: item.retryAt, error: item.error };
	}
	let recovery: SubagentExecutionState["recovery"];
	if (data.recovery !== undefined) {
		const item = record(data.recovery);
		if (!item || !time(item.requestedAt) || typeof item.requestedBy !== "string" || (item.startedAt !== undefined && !time(item.startedAt))) return undefined;
		recovery = { requestedAt: item.requestedAt, requestedBy: item.requestedBy, ...(item.startedAt !== undefined ? { startedAt: item.startedAt } : {}) };
	}
	...
	return snapshot({
		phase: data.phase, since: data.since, updatedAt: data.updatedAt, run: data.run,
		startedAt: data.startedAt, lastActivityAt: data.lastActivityAt, retries: data.retries, retryWaitMs: data.retryWaitMs,
		...
		...(retry ? { retry } : {}), ...(recovery ? { recovery } : {}), ...(notification ? { notification } : {}),
