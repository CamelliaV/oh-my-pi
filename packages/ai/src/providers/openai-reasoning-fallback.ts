...
const REASONING_EFFORT_FIELD_PATTERN =
	/reasoning[_. ]?effort|reasoning value|(?:valid|supported|allowed) (?:levels?|values?)|始终思考|思考模式|(?:不支持|无法|不能)(?:开启|关闭)?思考|(?:不支持|仅支持|只支持)[^\n]{0,40}?(?:思考|reasoning)|(?:不支持|仅支持|只支持)[^\n]{0,20}?(?:none|minimal|low|medium|high|xhigh|max)/i;
...
function isInvalidReasoningEffortError(
	...
	// Chinese gateways phrase the same rejection without the English field or
	// verdict vocabulary (`不支持思考` / `仅支持 "low"` style): the rejected value
	// named beside a Chinese verdict qualifies even without a field mention.
	if (
		new RegExp(
